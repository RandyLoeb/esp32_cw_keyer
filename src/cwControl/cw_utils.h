...
public:
  CwControl(std::queue<PaddlePressDetection *> *ditsNdahQueue)
  {
  ...
  {
    char ascii = this->convert_cw_number_to_ascii(number_in);

#if defined OPTION_PROSIGN_SUPPORT
    //might be a special prosign
    String prosign = String(this->convert_prosign(ascii));
